...
class BiomechanicsAnalyzer:
    def __init__(self, root):
        ...
        # Custom 셋팅 폴더 생성
        self.indiv_dir = os.path.join(self.setting_dir, "Custom")
        ...
        self.sport_combo = ttk.Combobox(self.tab_analysis, textvariable=self.sport_var, values=["Golf", "Baseball", "Workout", "Custom"], state="readonly")
        ...
        def update_analysis_options(event):
            ...
            elif self.sport_var.get() == "Custom":
                self.load_custom_analyses() 
        ...
        ask_donate_lbl = tk.Label(bottom_frame, text="Update & Ask & Donate", font=("Helvetica", 9, "underline"), fg="blue", cursor="hand2")
    ...
    def start_analysis(self, mode="video"):
        ...
        # [추가] Custom 모드일 때 JSON에 적힌 관절을 UI에 자동 체크 및 활성화
        if self.sport_var.get() == "Custom" and self.analysis_var.get() in self.custom_analyses_data:
            analyses_to_run = self.custom_analyses_data[self.analysis_var.get()]
            req_names = set()
            for cfg in analyses_to_run:
                if "target_points" in cfg: req_names.update(cfg["target_points"])
                if "target_point" in cfg: req_names.add(cfg["target_point"])
            
            for r_name in req_names:
                for k, v in self.landmark_names.items():
                    if v.split(". ")[-1].replace(" ", "_") == r_name:
                        if k not in self.selected_keys:
                            self.selected_keys.append(k)
                            self.checkbox_vars[k].set(True) 
    ...
    def update_reference_image(self, event=None):
        ...
        for ext in extensions:
            if sport == "Custom" and analysis in self.custom_analyses_data:
    ...
    def processing_thread(self):
        ...
        while self.is_running:
            ...
            # --------------------- Custom (CUSTOM JSON) ---------------------
            elif sport == "Custom" and analysis in self.custom_analyses_data:
                ...
                for custom_cfg in analyses_to_run:
                    ...
                    # 2. 각도(angle) 측정: 3개의 관절이 이루는 각도 계산
                        ...
                        if len(target_pts) == 3:
                            ...
                            # [추가] get()의 기본값을 np.nan으로 설정하고, dtype을 float으로 강제하여 에러 방지
                            p1 = np.array([frame_data.get(f"{p1_name}_X", np.nan), frame_data.get(f"{p1_name}_Y", np.nan)], dtype=float)
                            p2 = np.array([frame_data.get(f"{p2_name}_X", np.nan), frame_data.get(f"{p2_name}_Y", np.nan)], dtype=float)
                            p3 = np.array([frame_data.get(f"{p3_name}_X", np.nan), frame_data.get(f"{p3_name}_Y", np.nan)], dtype=float)
    ...
    def generate_pdf_report(self, df, name_part, sport, analysis):
        ...
        # Custom Workout일 때는 Sport 표시 생략
        header_prefix = "" if sport in ["Custom", "Workout"] else f"Sport: {sport} | "
        ...
        # 4-1. Custom (Custom JSON) 모드 (JSON 내 여러 분석 존재 시 다중 페이지 생성)
        elif sport == "Custom" and analysis in self.custom_analyses_data:
            with PdfPages(pdf_path) as pdf:
                ...
                # 만약 분석된 페이지가 하나도 없다면 안내 페이지 생성
                if pages_generated == 0:
                    fig_err = plt.figure(figsize=(8.27, 11.69))
                    plt.text(0.5, 0.5, "Analysis Failed.\n\nNo movement data found for custom points.", 
    ...
    def finish_analysis(self):
        ...
        for col in df.columns:
            # "Param_Name"이 포함된 칼럼은 제외하도록 조건 추가
            if col not in ["Time(sec)", "Error_Flag", "Analysis_Param_Name", "Measured_Value", "Target_Value", "Rep_Count", "Is_Active_Phase", "Start_Time"] and "Param_Name" not in col:
                # 데이터가 존재하고 문자열(object)이 아닐 때만 수학 필터 적용
        ...
        custom_cols = [c for c in df.columns if "Measured_Value" in c or "Target_Value" in c or "Param_Name" in c]
        
        # final_col_order 끝에 + custom_cols 를 추가해 줍니다.
        final_col_order = base_cols + ["Body_Speed_px_s", "Step_Width_px"] + coord_cols + [c for c in df.columns if "Angle" in c or "Velocity" in c or "Acceleration" in c or "Speed" in c] + custom_cols
